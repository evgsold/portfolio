...
export default function HeroClient({
  ...
  const featuredProjects = [
    { id: 1, title: 'Charming.by', tech: 'Next.js', image: '/projects/charming.png' },
    { id: 2, title: '3dfabriq', tech: 'React', image: '/projects/3dfabriq.png' },
  ];
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-end mb-12">
            ...
            <Link href={`/${locale}/portfolio`} className="text-[rgb(var(--primary))] font-bold hover:underline uppercase tracking-widest text-sm">
              {tPortfolio('allProjects')} →
            </Link>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {featuredProjects.map((project) => (
              <Link href={`/portfolio`} key={project.id} className="block group">
                <motion.div
                  whileHover={{ y: -10 }}
                  className="relative !rounded-none border border-[rgb(var(--border-color))] overflow-hidden h-full flex flex-col"
                >
