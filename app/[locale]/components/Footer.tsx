...
export default function Footer({ locale }: FooterProps) {
  ...
  return (
    <footer ref={footerRef} className="site-footer relative overflow-hidden border-t border-[rgb(var(--border-color))]">
      ...
      <motion.div style={{ y, opacity }} className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 z-10">
          ...
          <div>
            ...
            <ul className="space-y-4">
              {['home', 'about', 'portfolio', 'contact'].map((item) => (
                <li key={item}>
                  <a href={`/${locale}/${item === 'home' ? '' : item}`} className="footer-link text-lg font-medium hover:translate-x-2 inline-block transition-transform">
                    {tNav(item)}
                  </a>
                </li>
              ))}
        ...
        <div className="mt-20 pt-8 border-t border-[rgb(var(--border-color))] flex flex-col md:flex-row justify-between items-center gap-4">
          <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="text-xs font-bold uppercase tracking-widest hover:text-[rgb(var(--primary))] transition-colors">
            {tNav('backToTop')} ↑
          </button>
        </div>
      </motion.div>
